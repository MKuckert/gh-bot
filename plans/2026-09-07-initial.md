...
```
cron (hourly) → run.sh   ← the cron target
...
- **Code location:** repo root: `run.sh`, `token.sh`, `README.md`; modules in `src/`: `bot.mjs`, `auth.mjs`, `llm.mjs` (launchd template was dropped — cron is the scheduler).
...
| Private key PEM | `key.pem` at repo root (gitignored, `chmod 600`) | shown once at generation; never committed |
...
5. **M4 — Scheduling + ops (DONE 2026-09-06):** `run.sh` hardened as the cron target (PATH, flock overlap guard, fail-loud credential check); exact cron line + ops runbook in `README.md`. Operator installs the cron entry.
